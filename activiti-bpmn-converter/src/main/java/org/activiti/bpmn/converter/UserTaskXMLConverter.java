  ...
  /** default attributes taken from bpmn spec and from activiti extension */
	...
	@Override
	  ...
	  protected BaseElement convertXMLToElement(XMLStreamReader xtr, BpmnModel model) throws Exception {
	    ...
	    boolean condition = StringUtils.isNotEmpty(formKey) && model.getUserTaskFormTypes() != null && model.getUserTaskFormTypes().contains(formKey);
		if (condition) {
	...
	protected void writeCustomIdentities(UserTask userTask,String identityType, Set<String> users, Set<String> groups, XMLStreamWriter xtw) throws Exception {
	    ...
	    if (users!=null) {
	      users.forEach(userId -> identityList.add(new StringBuilder().append("user(").append(userId).append(")").toString()));
	    }
	    
	    if (groups!=null) {
	      groups.forEach(groupId -> identityList.add(new StringBuilder().append("group(").append(groupId).append(")").toString()));
	    }
