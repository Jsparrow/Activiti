  ...
  public static void writeBPMNDI(BpmnModel model, XMLStreamWriter xtw) throws Exception {
    ...
    for (String elementId : model.getLocationMap().keySet()) {
      ...
      if (model.getFlowElement(elementId) != null || model.getArtifact(elementId) != null || model.getPool(elementId) != null || model.getLane(elementId) != null) {
        ...
        xtw.writeAttribute(ATTRIBUTE_DI_HEIGHT, Double.toString(graphicInfo.getHeight()));
        xtw.writeAttribute(ATTRIBUTE_DI_WIDTH, Double.toString(graphicInfo.getWidth()));
        xtw.writeAttribute(ATTRIBUTE_DI_X, Double.toString(graphicInfo.getX()));
        xtw.writeAttribute(ATTRIBUTE_DI_Y, Double.toString(graphicInfo.getY()));
        xtw.writeEndElement();
    ...
    for (String elementId : model.getFlowLocationMap().keySet()) {
      ...
      if (model.getFlowElement(elementId) != null || model.getArtifact(elementId) != null || model.getMessageFlow(elementId) != null) {
        ...
        if (labelGraphicInfo != null && hasName) {
          ...
          xtw.writeStartElement(OMGDC_PREFIX, ELEMENT_DI_BOUNDS, OMGDC_NAMESPACE);
          xtw.writeAttribute(ATTRIBUTE_DI_HEIGHT, Double.toString(labelGraphicInfo.getHeight()));
          xtw.writeAttribute(ATTRIBUTE_DI_WIDTH, Double.toString(labelGraphicInfo.getWidth()));
          xtw.writeAttribute(ATTRIBUTE_DI_X, Double.toString(labelGraphicInfo.getX()));
          xtw.writeAttribute(ATTRIBUTE_DI_Y, Double.toString(labelGraphicInfo.getY()));
          xtw.writeEndElement();
          xtw.writeEndElement();
        }
