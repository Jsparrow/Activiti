  ...
  public static class SuspensionStateUtil {
    ...
    public static void setSuspensionState(ProcessDefinitionEntity processDefinitionEntity, SuspensionState state) {
      if (processDefinitionEntity.getSuspensionState() == state.getStateCode()) {
        throw new ActivitiException(new StringBuilder().append("Cannot set suspension state '").append(state).append("' for ").append(processDefinitionEntity).append("': already in state '").append(state).append("'.")
				.toString());
      }
    ...
    public static void setSuspensionState(ExecutionEntity executionEntity, SuspensionState state) {
      if (executionEntity.getSuspensionState() == state.getStateCode()) {
        throw new ActivitiException(new StringBuilder().append("Cannot set suspension state '").append(state).append("' for ").append(executionEntity).append("': already in state '").append(state).append("'.")
				.toString());
      }
    ...
    public static void setSuspensionState(TaskEntity taskEntity, SuspensionState state) {
      if (taskEntity.getSuspensionState() == state.getStateCode()) {
        throw new ActivitiException(new StringBuilder().append("Cannot set suspension state '").append(state).append("' for ").append(taskEntity).append("': already in state '").append(state).append("'.")
				.toString());
      }
