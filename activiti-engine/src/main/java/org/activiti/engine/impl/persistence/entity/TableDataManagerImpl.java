  ...
  private static Logger log = LoggerFactory.getLogger(TableDataManagerImpl.class);
	...
	@Override
	  public Map<String, Long> getTableCount() {
	    ...
	    try {
	      getTablesPresentInDatabase().forEach(tableName -> tableCount.put(tableName, getTableCount(tableName)));
	    ...
	    try {
	      ...
	      try {
	        ...
	        if ("oracle".equals(getDbSqlSession().getDbSqlSessionFactory().getDatabaseType())) {
	          tableNameFilter = new StringBuilder().append(databaseTablePrefix).append("ACT").append(databaseMetaData.getSearchStringEscape()).append("_%").toString();
	        }
