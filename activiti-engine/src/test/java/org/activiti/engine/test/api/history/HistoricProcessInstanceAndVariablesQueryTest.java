...
public class HistoricProcessInstanceAndVariablesQueryTest extends PluggableActivitiTestCase {
  ...
  private static String processDefinitionKey2 = "oneTaskProcess2";
  private static String processDefinitionName2 = "oneTaskProcess2Name";
  private static String processDefinitionCategory2 = "org.activiti.enginge.test.api.runtime.2Category";
  private static String processDefinitionKey3 = "oneTaskProcess3";
...
protected void setUp() throws Exception {
    ...
    for (int i = 0; i < 4; i++) {
      processInstanceIds.add(runtimeService.startProcessInstanceByKey(processDefinitionKey, Integer.toString(i), startMap).getId());
      if (i == 0) {
    ...
    processInstanceIds.add(runtimeService.startProcessInstanceByKey(processDefinitionKey2, "1", startMap).getId());
    
    startMap.clear();
    startMap.put("casetest", "MyTest");
    processInstanceIds.add(runtimeService.startProcessInstanceByKey(processDefinitionKey3, "1", startMap).getId());
  }
...
protected void tearDown() throws Exception {
  ...
  public void testQuery() {
	...
	instanceList = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().processDefinitionKey(processDefinitionKey).list();
	...
	assertEquals(processDefinitionKey, instanceList.get(0).getProcessDefinitionKey());
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().processDefinitionKey(processDefinitionKey2).singleResult();
	variableMap = processInstance.getProcessVariables();
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	...
	assertEquals("test2", variableMap.get("test2"));
	assertEquals(4, historyService.createHistoricProcessInstanceQuery().variableValueEquals("test", "test").includeProcessVariables().count());
	instanceList = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().processDefinitionKey(processDefinitionKey).listPage(1, 2);
	...
	assertEquals("test", variableMap.get("test"));
	assertEquals("test2", variableMap.get("test2"));
	instanceList = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().processDefinitionKey(processDefinitionKey).listPage(3, 4);
	assertEquals(1, instanceList.size());
	processInstance = instanceList.get(0);
	variableMap = processInstance.getProcessVariables();
	assertEquals(2, variableMap.size());
	assertEquals("test", variableMap.get("test"));
	assertEquals("test2", variableMap.get("test2"));
	instanceList = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().processDefinitionKey(processDefinitionKey).listPage(4, 2);
	assertEquals(0, instanceList.size());
  ...
  public void testQueryByprocessDefinition() {
	...
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .variableValueEquals("anothertest", 123).processDefinitionName(processDefinitionName2).singleResult();
	variableMap = processInstance.getProcessVariables();
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	assertEquals(processDefinitionName2, processInstance.getProcessDefinitionName());
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .variableValueEquals("test", "test").processDefinitionName(processDefinitionName2).singleResult();
	assertNull(processInstance);
	// ProcessDefinitionCategory
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .variableValueEquals("anothertest", 123).processDefinitionCategory(processDefinitionCategory2).singleResult();
	variableMap = processInstance.getProcessVariables();
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .variableValueEquals("test", "test").processDefinitionCategory(processDefinitionCategory2).singleResult();
	assertNull(processInstance);
  }
  ...
  public void testOrQuery() {
	...
	List<HistoricProcessInstance> instanceList = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().or()
          .processDefinitionKey(processDefinitionKey).processDefinitionId("undefined").endOr().list();
	assertEquals(4, instanceList.size());
	processInstance = instanceList.get(0);
	variableMap = processInstance.getProcessVariables();
	assertEquals(2, variableMap.size());
	assertEquals("test", variableMap.get("test"));
	assertEquals("test2", variableMap.get("test2"));
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().or().processDefinitionKey(processDefinitionKey2).processDefinitionId("undefined").endOr()
          .singleResult();
	variableMap = processInstance.getProcessVariables();
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	...
	assertEquals("test", variableMap.get("test"));
	assertEquals("test2", variableMap.get("test2"));
	instanceList = historyService.createHistoricProcessInstanceQuery().includeProcessVariables().or().processDefinitionKey(processDefinitionKey).processDefinitionId("undefined").endOr()
          .listPage(1, 2);
	assertEquals(2, instanceList.size());
	processInstance = instanceList.get(0);
	variableMap = processInstance.getProcessVariables();
  ...
  public void testOrQueryByprocessDefinition() {
	...
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	historicprocessInstanceQuery.list().forEach(processInstance -> assertEquals(deploymentId, processInstance.getDeploymentId()));
	HistoricProcessInstance processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .or().variableValueEquals("anothertest", "invalid").deploymentId("invalid").endOr().singleResult();
	assertNull(processInstance);
	// ProcessDefinitionName
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .or().variableValueEquals("anothertest", "invalid").processDefinitionName(processDefinitionName2).endOr().singleResult();
	variableMap = processInstance.getProcessVariables();
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	assertEquals(processDefinitionName2, processInstance.getProcessDefinitionName());
	...
	// ProcessDefinitionCategory
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
	        .or().variableValueEquals("anothertest", "invalid").processDefinitionCategory(processDefinitionCategory2).endOr().singleResult();
	variableMap = processInstance.getProcessVariables();
	assertEquals(1, variableMap.size());
	assertEquals(123, variableMap.get("anothertest"));
	processInstance = historyService.createHistoricProcessInstanceQuery().includeProcessVariables()
