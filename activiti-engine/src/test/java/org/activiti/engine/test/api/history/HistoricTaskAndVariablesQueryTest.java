...
public class HistoricTaskAndVariablesQueryTest extends PluggableActivitiTestCase {
    ...
    public void testCandidate() {
		...
		assertEquals(1,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(KERMIT,
		                                                                           KERMITSGROUPS).taskCandidateGroupIn(Collections.singletonList("management")).list();
		assertEquals(3,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(GONZO,
		                                                                           GONZOSGROUPS).taskCandidateGroupIn(Collections.singletonList("management")).list();
		assertEquals(1,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(GONZO,
		                                                                           GONZOSGROUPS).taskCandidateGroupIn(Collections.singletonList("invalid")).list();
		assertEquals(0,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateGroupIn(groups).list();
		assertEquals(1,
    ...
    public void testCandidateWithUserGroupProxy() {
		...
		assertEquals(1,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(KERMIT).taskCandidateGroupIn(Collections.singletonList("management")).list();
		assertEquals(3,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(GONZO).taskCandidateGroupIn(Collections.singletonList("management")).list();
		assertEquals(1,
		             tasks.size());
		tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(GONZO).taskCandidateGroupIn(Collections.singletonList("invalid")).list();
		assertEquals(0,
		             tasks.size());
    }
