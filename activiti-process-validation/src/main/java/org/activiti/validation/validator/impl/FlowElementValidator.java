...
public class FlowElementValidator extends ProcessLevelValidator {
	...
	protected void executeValidation(BpmnModel bpmnModel, Process process, List<ValidationError> errors) {
		process.getFlowElements().stream().filter(flowElement -> flowElement instanceof Activity).map(flowElement -> (Activity) flowElement).forEach(activity -> {
	...
	protected void handleConstraints(Process process, Activity activity, List<ValidationError> errors) {
		if (activity.getId() != null && activity.getId().length() > ID_MAX_LENGTH) {
			addError(errors, Problems.FLOW_ELEMENT_ID_TOO_LONG, process, activity,
					new StringBuilder().append("The id of a flow element must not contain more than ").append(ID_MAX_LENGTH).append(" characters").toString());
		}
	}
	...
	protected void handleMultiInstanceLoopCharacteristics(Process process, Activity activity, List<ValidationError> errors) {
		...
		boolean condition = multiInstanceLoopCharacteristics != null && StringUtils.isEmpty(multiInstanceLoopCharacteristics.getLoopCardinality())
  ...
  addError(errors, Problems.MULTI_INSTANCE_MISSING_COLLECTION, process, activity,
	...
	protected void handleDataAssociations(Process process, Activity activity, List<ValidationError> errors) {
	  if (activity.getDataInputAssociations() != null) {
	  	activity.getDataInputAssociations().stream().filter(dataAssociation -> StringUtils.isEmpty(dataAssociation.getTargetRef())).forEach(dataAssociation -> addError(errors, Problems.DATA_ASSOCIATION_MISSING_TARGETREF, process, activity,
				"Targetref is required on a data association"));
	  }
	  if (activity.getDataOutputAssociations() != null) {
	  	activity.getDataOutputAssociations().stream().filter(dataAssociation -> StringUtils.isEmpty(dataAssociation.getTargetRef())).forEach(dataAssociation -> addError(errors, Problems.DATA_ASSOCIATION_MISSING_TARGETREF, process, activity,
				"Targetref is required on a data association"));
	  }
  }
